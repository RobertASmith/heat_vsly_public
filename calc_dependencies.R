...
# create dataframe with two columns, male and female.
...
v.female <- pf_age80[rownames(who_mort),"perc_fmle"] # vector of sex at age 85
...
f.dlyr <- function(country,d.r){
  ...
  v.Ps <- (1 - who_mort[country,1:ncol(who_mort)] %>% rep(each=5))[-1]
  ...
  m.lyr[(85:(85+s.clyr85-1)),]  <-  matrix(m.lyr[84,],ncol = 84 ,nrow = s.clyr85,byrow="T")  
...
dlyr <- matrix(data = NA,
               nrow = nrow(m.C_dlyr),
               ncol = 17)
rownames(dlyr) <- rownames(countries)
...
countries$VSLY <- countries$VSL / m.C_dlyr[,50]      # simply the VSL divided by the discounted life years remaining at 50 
